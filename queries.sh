...
echo "$($PSQL "SELECT name from teams left join games on games.winner_id = teams.team_id or games.opponent_id = teams.team_id WHERE round= 'Eighth-Final' AND year=2014 group by name order by name asc")"
